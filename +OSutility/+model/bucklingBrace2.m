classdef bucklingBrace2 < handle
    ...
    properties
        ...
        iRigidEle = [];         % rigid element at brace end i
        jRigidEle = [];         % rigid element at brace end j
        iBrNode = [];           % brace node at end i
        jBrNode = [];           % brace node at end j
    ...
    methods
        ...
        function obj = bucklingBrace2(db, sec, rigidSec, geomTransf, geomTransfR, iNode, jNode, varargin)
            ...
            if ~isempty(obj.lri)
                ...
                db.addNode(gpi);
%                 db.addFix( OpenSees.model.constraint.equalDOF(cur_left_node, gpi, 3) );
                
                newEleTag = db.getEleTag(cur_left_node.tag);
                obj.iRigidEle = OpenSees.model.element.forceBeamColumn(newEleTag, cur_left_node, gpi,obj.geomTransfR, obj.rigidSec);
                db.addElement(obj.iRigidEle);
            ...
            if ~isempty(obj.lrj)
                ...
                db.addNode(gpj);
%                 db.addFix( OpenSees.model.constraint.equalDOF(cur_right_node, gpj, 3) );
                
                newEleTag = db.getEleTag(cur_right_node.tag);
                obj.jRigidEle = OpenSees.model.element.forceBeamColumn(newEleTag, cur_right_node, gpj, obj.geomTransfR, obj.rigidSec);
                db.addElement(obj.jRigidEle);
...
%                 db.addFix( OpenSees.model.constraint.equalDOF(cur_left_node, bri, 6) );
            ...
            if ~isempty(obj.lgpj)
                if any(ismember(p.UsingDefaults,'jGPSec')) == 0
