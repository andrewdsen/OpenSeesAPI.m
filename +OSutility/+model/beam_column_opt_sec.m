classdef beam_column_opt_sec < handle
    ...
    methods
        ...
        function obj = beam_column_opt_sec(db, sec, sec_meta, geom_transf, geom_transf_rig, node_i, node_j, varargin)
            ...
            addOptional(p, 'sec_i', obj.sec_i);
            addOptional(p, 'sec_j', obj.sec_j);
            ...
            if any(ismember(p.UsingDefaults, 'sec_i')) == 0 && ~isempty(p.Results.sec_i)
                obj.sec_i = p.Results.sec_i;
                ...
                new_ele_tag = db.getEleTag(cur_left_node.tag);
                obj.spring_ele_i = OpenSees.model.element.zeroLengthSection(new_ele_tag, cur_left_node, i_dum, obj.sec_i, 'x', [dx dy dz], 'yp', obj.vert_vec);
            ...
            if any(ismember(p.UsingDefaults, 'sec_j')) == 0 && ~isempty(p.Results.sec_j)
                obj.sec_j = p.Results.sec_j;
                ...
                new_ele_tag = db.getEleTag(cur_right_node.tag);
                obj.spring_ele_j = OpenSees.model.element.zeroLengthSection(new_ele_tag, cur_right_node, j_dum, obj.sec_j, 'x', -[dx dy dz], 'yp', obj.vert_vec);
            ...
            new_ele_tag = db.getEleTag(cur_left_node.tag);
            obj.bc_ele = OpenSees.model.element.forceBeamColumn(new_ele_tag, cur_left_node, cur_right_node, obj.geom_transf, obj.sec, 'maxIters', 250, 'np', 5, 'mass', obj.mass_dens, 'tol', 10);
